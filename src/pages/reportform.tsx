...
export const ReportForm: React.FC = () => {
  ...
  // LocalStorageのキー生成用ヘルパー（ユーザーIDを含めてセキュリティを確保）
  const getAutoSaveKey = (storeId: string, date: string, operationType: OperationType) => {
    const userId = user?.id || 'anonymous'
    return `autoSave_${userId}_${storeId}_${date}_${operationType}`
  }

  // 自動保存用のヘルパー関数
  const saveToLocalStorage = (data: FormState) => {
    if (isEditMode || !data.storeId || data.storeId === 'all') return

    const key = getAutoSaveKey(data.storeId, data.date, data.operationType)
    const saveData = {
      ...data,
  ...
  const restoreFromLocalStorage = (storeId: string, date: string, operationType: OperationType) => {
    const key = getAutoSaveKey(storeId, date, operationType)
  ...
  // 自動保存データをクリア
  const clearAutoSaveData = (storeId: string, date: string, operationType: OperationType) => {
    const key = getAutoSaveKey(storeId, date, operationType)
    localStorage.removeItem(key)
  }

  // 古い自動保存データをクリーンアップ（7日以上前のデータ）
  const cleanupOldAutoSaveData = () => {
  ...
  // ページ読み込み時に自動保存データを復元
  useEffect(() => {
    if (!isEditMode && form.storeId && form.storeId !== 'all' && form.date && !hasRestoredData) {
      const restoredData = restoreFromLocalStorage(form.storeId, form.date, form.operationType)

      if (restoredData) {
        const hasData =
          (restoredData.sales || 0) > 0 ||
          (restoredData.purchase || 0) > 0 ||
          (restoredData.laborCost || 0) > 0 ||
          (restoredData.customers || 0) > 0 ||
          (restoredData.reportText || '').trim() !== ''
  ...
  }, [form.storeId, form.date, form.operationType, isEditMode, hasRestoredData])

  // フォームデータの変更を監視して自動保存（デバウンス付き）
  useEffect(() => {
    if (isEditMode || !form.storeId || form.storeId === 'all') return
  ...
  // 営業時間帯切り替え時もLocalStorageに保存（無効化）
  // useEffect(() => {
  //   if (!isEditMode && form.storeId && form.storeId !== 'all') {
  //     if (form.operationType === 'lunch' && tempLunchData) {
  //       const lunchKey = getAutoSaveKey(form.storeId, form.date, 'lunch')
  //       localStorage.setItem(lunchKey, JSON.stringify({ ...tempLunchData, savedAt: new Date().toISOString() }))
  //     } else if (form.operationType === 'dinner' && tempDinnerData) {
  //       const dinnerKey = getAutoSaveKey(form.storeId, form.date, 'dinner')
  //       localStorage.setItem(dinnerKey, JSON.stringify({ ...tempDinnerData, savedAt: new Date().toISOString() }))
  //     }
  //   }
  // }, [tempLunchData, tempDinnerData, form.storeId, form.date, form.operationType, isEditMode])

  // ページ離脱時の警告
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      if (hasUnsavedData && !saved && !isEditMode) {
        e.preventDefault()
        e.returnValue = ''
      }
    }
  ...
  }, [hasUnsavedData, saved, isEditMode])
  ...
  useEffect(() => {
    if (isEditMode && editingReportId) {
      const loadReport = async () => {
        ...
        if (editingReportId.startsWith('local-') && user?.id) {
          const localReports: DailyReportData[] = JSON.parse(localStorage.getItem(`userReports_${user.id}`) || '[]')
  ...
  useEffect(() => {
    if (!isEditMode && form.storeId && user?.id) {
      const last = localStorage.getItem(`lastReport_${user.id}_${form.storeId}_${form.operationType}`)
      if (last) {
  ...
  }, [form.storeId, form.operationType, isEditMode, user?.id])
  ...
  const handleOperationTypeChange = (newOperationType: OperationType) => {
    ...
    // 現在の営業時間帯に応じて一時保存（メモリ＋LocalStorage）
    if (form.operationType === 'lunch') {
      setTempLunchData(currentData)
      const lunchKey = getAutoSaveKey(form.storeId, form.date, 'lunch')
      localStorage.setItem(lunchKey, JSON.stringify({ ...currentData, savedAt: new Date().toISOString() }))
    } else if (form.operationType === 'dinner') {
      setTempDinnerData(currentData)
      const dinnerKey = getAutoSaveKey(form.storeId, form.date, 'dinner')
      localStorage.setItem(dinnerKey, JSON.stringify({ ...currentData, savedAt: new Date().toISOString() }))
    }

    // 切り替え先のデータを復元（LocalStorage優先）
    ...
    const savedData = restoreFromLocalStorage(form.storeId, form.date, newOperationType)
    if (savedData) {
      restoredData = savedData
    } else if (newOperationType === 'lunch' && tempLunchData) {
  ...
  // 日報分析を実行（AIフィードバック）
  // Note: Edge Function で OPENAI_API_KEY が未設定の場合は 500 エラーを返しますが、
  // このエラーは日報保存には影響せず、デフォルトのフィードバックを表示します
  const analyzeReport = async (reportData: { date: string; sales: number; customer_count: number; note?: string; weather?: string }) => {
    ...
    try {
      // Step 1: AI使用回数チェック（認証済みユーザーのみ）
      const { data: { session } } = await supabase.auth.getSession()

      if (session?.access_token && selectedStore) {
        const usageCheckUrl = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/ai-usage-proxy`
        ...
        if (!usageResponse.ok) {
          ...
          if (usageResponse.status === 429) {
            throw new Error(usageError.error || 'AI使用回数の上限に達しました')
          }

          console.warn('AI使用回数チェックエラー:', usageError)
        }
      }
      ...
      if (!response.ok) {
        ...
        console.warn('AI分析API エラー:', response.status, errorData)
        throw new Error(errorData.error || '分析APIの呼び出しに失敗しました')
      }
      ...
      console.error('日報分析エラー（日報保存には影響しません）:', error)
  ...
  const submit = async () => {
    ...
    const selectedStore = accessibleStores.find(s => s.id === form.storeId)
    if (!selectedStore) {
    ...
    const saveToLocal = () => {
      if (!user?.id) return
      const key = `userReports_${user.id}`
    ...
    try {
      ...
      if (!isSupabaseReady()) {
        ...
        if (!isEditMode) {
          ...
          if (user?.id) {
            localStorage.removeItem(`lastReport_${user.id}_${form.storeId}_${form.operationType}`)
          }
        }
      ...
      // Supabaseに保存または更新
      let data, error
      let existingReportId = isEditMode ? editingReportId : null
      ...
      if (!isEditMode) {
        try {
          const { data: existingReports } = await supabase
            .from('daily_reports')
            .select('id, sales, customers')
            .eq('date', form.date)
            .eq('store_id', form.storeId)
            .eq('operation_type', form.operationType)
            .maybeSingle()

          if (existingReports?.id) {
            console.log('📝 既存の日報を検出:', existingReports)

            // ユーザーに確認を求める
            const operationTypeText = form.operationType === 'lunch' ? 'ランチ' : 'ディナー'
            const confirmed = window.confirm(
              `この日付（${form.date}）・店舗・営業区分（${operationTypeText}）の日報はすでに存在します。\n\n` +
              `既存データ:\n` +
              `- 売上: ${formatCurrency(existingReports.sales || 0)}\n` +
              `- 客数: ${existingReports.customers || 0}名\n\n` +
              `上書きしてもよろしいですか？`
            )

            if (!confirmed) {
              console.log('⚠️ ユーザーが上書きをキャンセル')
              setSaving(false)
              return
            }

            console.log('✅ ユーザーが上書きを承認、更新モードに切り替え')
            existingReportId = existingReports.id
          }
        } catch (checkError) {
      ...
      if (existingReportId) {
        ...
        const result = await updateDailyReport(existingReportId, updates)
        data = result.data
        error = result.error
      ...
      if (error) {
        ...
        const wasUpdate = !!existingReportId
        console.log('✅ Supabaseに保存成功:', wasUpdate ? '(更新)' : '(新規作成)', data)
        ...
        if (!wasUpdate) {
          ...
          // LocalStorageの自動保存データもクリア
          clearAutoSaveData(form.storeId, form.date, form.operationType)
        }
        ...
        if (!wasUpdate) {
          ...
          // AIフィードバックを取得して表示（Best Effort - 失敗しても日報保存は成功扱い）
          // Note: Edge Function で OPENAI_API_KEY が未設定の場合は 500 を返すが、日報保存には影響しない
          analyzeReport({
            date: form.date,
            sales: form.sales,
            ...
            console.error('AI分析は失敗しましたが、日報は正常に保存されました:', err)
          })
        }
      ...
      // 編集モードまたは既存データを上書きした場合はダッシュボードに戻る
  ...
  return (
    <>
      <div className="max-w-4xl mx-auto space-y-4 px-2 sm:px-4 lg:px-6 pb-24 lg:pb-4">
        ...
        {/* データ復元通知バナー */}
        {showRestoreNotice && hasRestoredData && !isEditMode && (
          <div className="p-4 bg-amber-50 border-2 border-amber-300 rounded-lg shadow-sm">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-sm font-medium text-amber-900 mb-1">未保存のデータが見つかりました</p>
                <p className="text-xs text-amber-800">
                  前回入力途中だったデータが自動保存されています。このまま入力を続けるか、データをクリアして新規入力することができます。
                </p>
        ...
        {!isEditMode && lastAutoSaveTime && (
          <div className="flex items-center justify-between p-2 bg-green-50 border border-green-200 rounded-lg">
            ...
            <span className="text-xs text-green-700">
              {(() => {
                const now = new Date()
                const diffSec = Math.floor((now.getTime() - lastAutoSaveTime.getTime()) / 1000)
      ...
      <div className="lg:relative fixed lg:bottom-auto bottom-0 left-0 right-0 z-30 flex flex-col sm:flex-row gap-3 lg:sticky lg:bottom-4 bg-background p-4 pb-[max(1rem,env(safe-area-inset-bottom))] sm:pb-4 -mx-2 sm:mx-0 lg:p-0 lg:bg-transparent rounded-t-lg lg:rounded-lg border-t lg:border-none shadow-lg lg:shadow-none">
