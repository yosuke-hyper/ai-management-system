...
export const subscriptionService = {
  ...
  async getSubscriptionLimits(organizationId: string): Promise<SubscriptionLimits | null> {
    ...
    return {
      ...
      currentAiUsage: aiUsage?.current_usage || 0,
   ...
   * 契約店舗数のみを変更（プラン変更なし）
   * @param organizationId 組織ID
   * @param newContractedStores 新しい契約店舗数
   */
  async updateContractedStores(
    organizationId: string,
    newContractedStores: number
  ): Promise<{ priceImpact: { oldPrice: number; newPrice: number; difference: number } }> {
    console.log('🔧 updateContractedStores開始:', { organizationId, newContractedStores });

    if (newContractedStores < 1) {
      throw new Error('契約店舗数は1以上を指定してください');
    }

    if (newContractedStores > 100) {
      throw new Error('契約店舗数は100以下を指定してください。それ以上の店舗数が必要な場合はお問い合わせください');
    }
    ...
    const planName = currentSubscription.plan.name as PlanName;
    const billingCycle = currentSubscription.plan.billing_cycle as BillingCycle;
    ...
    const oldContractedStores = currentData?.contracted_stores || 1;
    const oldPricing = this.calculatePriceForStores(planName, oldContractedStores, billingCycle);
    const newPricing = this.calculatePriceForStores(planName, newContractedStores, billingCycle);

    const oldPrice = billingCycle === 'monthly' ? oldPricing.monthlyPrice : oldPricing.annualPrice;
    const newPrice = billingCycle === 'monthly' ? newPricing.monthlyPrice : newPricing.annualPrice;

    const { error } = await supabase
      .from('organization_subscriptions')
      .update({
        contracted_stores: newContractedStores,
        updated_at: new Date().toISOString()
      })
      .eq('id', currentSubscription.id);

    if (error) {
      console.error('❌ 契約店舗数の更新失敗:', error);
      throw new Error(`契約店舗数の更新に失敗しました: ${error.message}`);
    }

    console.log('✅ 契約店舗数を更新:', { oldContractedStores, newContractedStores });
