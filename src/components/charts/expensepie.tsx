...
const ExpensePieComponent: React.FC<ExpensePieProps> = ({
  ...
  const expenseData = React.useMemo(() => {
    ...
    // その他経費の合計を計算（人件費を除く）
    // 実際のレポートデータを優先し、0の場合のみ参考経費を適用
    let finalUtilities = totals.utilities
    let finalRent = totals.rent
    let finalConsumables = totals.consumables
    let finalPromotion = totals.promotion
    let finalCleaning = totals.cleaning
    let finalMisc = totals.misc
    let finalCommunication = totals.communication
    let finalOthers = totals.others

    // 参考経費が提供されている場合、実際の経費が0の項目のみ参考経費を適用
    if (expenseBaseline && effectiveDays > 0) {
      if (finalUtilities === 0) finalUtilities = (expenseBaseline.utilities || 0) * effectiveDays
      if (finalRent === 0) finalRent = (expenseBaseline.rent || 0) * effectiveDays
      if (finalConsumables === 0) finalConsumables = (expenseBaseline.consumables || 0) * effectiveDays
      if (finalPromotion === 0) finalPromotion = (expenseBaseline.promotion || 0) * effectiveDays
      if (finalCleaning === 0) finalCleaning = (expenseBaseline.cleaning || 0) * effectiveDays
      if (finalMisc === 0) finalMisc = (expenseBaseline.misc || 0) * effectiveDays
      if (finalCommunication === 0) finalCommunication = (expenseBaseline.communication || 0) * effectiveDays
      if (finalOthers === 0) finalOthers = (expenseBaseline.others || 0) * effectiveDays
    }

    const otherExpensesExcludingLabor = finalUtilities + finalRent + finalConsumables +
                                        finalPromotion + finalCleaning + finalMisc +
                                        finalCommunication + finalOthers
