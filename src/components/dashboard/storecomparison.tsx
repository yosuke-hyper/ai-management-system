          ...
          <div className="block md:hidden p-4 space-y-3">
            <div className="flex items-center gap-2 mb-3 flex-wrap">
              ...
              <button
                ...
                className={`text-xs px-2 py-1 rounded ${sortBy === 'sales' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}
              >
                売上
              </button>
              <button
                onClick={() => handleSort('profit')}
                className={`text-xs px-2 py-1 rounded ${sortBy === 'profit' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}
              >
                利益
              </button>
              <button
                onClick={() => handleSort('profitMargin')}
                className={`text-xs px-2 py-1 rounded ${sortBy === 'profitMargin' ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-600'}`}
              >
                利益率
              </button>
            </div>
            {sortedStores.map((store, index) => (
              <div key={store.id} className="border border-gray-200 rounded-lg p-3 bg-white">
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-blue-50 rounded p-2">
                    <div className="text-xs text-blue-600 mb-0.5">売上</div>
                    <div className="text-sm font-bold text-blue-700">{formatCurrency(store.sales)}</div>
                  </div>
                  <div className={`rounded p-2 ${store.profit >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                    <div className={`text-xs mb-0.5 ${store.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>利益</div>
                    <div className={`text-sm font-bold ${store.profit >= 0 ? 'text-green-700' : 'text-red-700'}`}>
                      {formatCurrency(store.profit)}
                    </div>
                  </div>
                  <div className={`rounded p-2 ${
                    store.profitMargin >= 15 ? 'bg-green-50' :
                    store.profitMargin >= 10 ? 'bg-yellow-50' : 'bg-red-50'
                  }`}>
                    <div className={`text-xs mb-0.5 ${
                      store.profitMargin >= 15 ? 'text-green-600' :
                      store.profitMargin >= 10 ? 'text-yellow-600' : 'text-red-600'
                    }`}>利益率</div>
                    <div className={`text-sm font-bold ${
                      store.profitMargin >= 15 ? 'text-green-700' :
                      store.profitMargin >= 10 ? 'text-yellow-700' : 'text-red-700'
                    }`}>
                ...
                <div className="mt-2 pt-2 border-t border-gray-100 flex items-center justify-between text-xs text-gray-500">
                  <span>報告数: {store.reportCount}件</span>
                  <span>日平均: {formatCurrency(store.averageDailySales)}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            <table className="w-full">
              <thead className="bg-gray-50">
                <tr>
                  ...
                  <th
                    className="hidden lg:table-cell px-4 lg:px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
