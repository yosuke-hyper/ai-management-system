          ...
          <>
            ...
            <div className="block sm:hidden space-y-3">
              <div className="flex items-center gap-2 mb-3 flex-wrap">
                ...
                >
                  期間 <SortIcon field="date" />
                </Button>
                <Button
                  variant={sortField === 'sales' ? 'default' : 'outline'}
                  size="sm"
                  onClick={() => handleSort('sales')}
                  className="text-xs h-7 px-2"
                >
                  売上 <SortIcon field="sales" />
                </Button>
                <Button
                  variant={sortField === 'profit' ? 'default' : 'outline'}
                  size="sm"
                  onClick={() => handleSort('profit')}
                  className="text-xs h-7 px-2"
                >
                  利益 <SortIcon field="profit" />
                </Button>
              </div>
                ...
                <div key={row.id} className="border border-gray-200 rounded-lg p-3 bg-white">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className={`text-xs font-medium px-2 py-0.5 rounded ${
                      row.profitMargin >= 15 ? 'bg-green-100 text-green-700' :
                      row.profitMargin >= 10 ? 'bg-yellow-100 text-yellow-700' : 'bg-red-100 text-red-700'
                    }`}>
                      利益率 {formatPercent(row.profitMargin)}
                    </span>
                  </div>
                  <div className="text-xs text-gray-500 mb-2">{row.storeName}</div>
                  <div className="grid grid-cols-3 gap-2 text-center">
                    <div className="bg-blue-50 rounded p-2">
                      <div className="text-xs text-blue-600 mb-0.5">売上</div>
                      <div className="text-sm font-bold text-blue-700">{formatCurrency(row.sales)}</div>
                    </div>
                    <div className="bg-red-50 rounded p-2">
                      <div className="text-xs text-red-600 mb-0.5">経費</div>
                      <div className="text-sm font-bold text-red-700">{formatCurrency(row.expenses)}</div>
                    </div>
                    <div className={`rounded p-2 ${row.profit >= 0 ? 'bg-green-50' : 'bg-red-50'}`}>
                      <div className={`text-xs mb-0.5 ${row.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>利益</div>
                      <div className={`text-sm font-bold ${row.profit >= 0 ? 'text-green-700' : 'text-red-700'}`}>
