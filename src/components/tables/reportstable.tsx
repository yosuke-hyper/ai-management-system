...
export const ReportsTable: React.FC<ReportsTableProps> = ({ reports, stores, onReportUpdate }) => {
  ...
  }, [reports, searchTerm, selectedStoreId, selectedMonth, deletedReportIds]);
  ...
  const visibleReportsCount = reports.filter(r => !deletedReportIds.has(r.id)).length;

  if (visibleReportsCount === 0) {
    return (
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      {/* データなしの場合 */}
      {filteredReports.length === 0 && visibleReportsCount > 0 && (
